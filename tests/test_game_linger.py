...
def _game(appid: str = "3041230", *, cpu: float = 40.0) -> dict:
    return {
        "id": appid,
        "name": "Windrose",
        "running": True,
        "primary_name": "Windrose.exe",
        "primary_pid": 12345,
        "cpu_pct": cpu,
        "rss_mb": 900.0,
        "tree_rss_mb": 1200.0,
...
def test_linger_expires_after_timeout():
    reset_game_linger()
    primary_active_game_with_linger({"3041230": _game()}, now=0.0)
...
def test_linger_clears_when_new_live_game():
    reset_game_linger()
    primary_active_game_with_linger({"3041230": _game()}, now=0.0)
    other = primary_active_game_with_linger({"730": _game("730", cpu=80)}, now=2.0)
